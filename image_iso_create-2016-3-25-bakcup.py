        ...
        if latest_build:
            for i in latest_build:
                for arch in architecture:
                    if i.find("RHEL-6") != -1:
                        jigdo_file_dir = "/tmp/redhat/rel-eng/%s/compose/Server/%s/jigdo" % (i,arch)
                    else:
                        if arch == "i386":
                            continue
                        jigdo_file_dir = "/tmp/redhat/rel-eng/%s/compose/Server/x86_64/jigdo" % i
                    ...
                    run_jigdo_cmd = subprocess.Popen(jigdo_cmd, shell=True, stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
def main():
    iso_dir = "/tmp/iso"
    nfs_dir = "/tmp/redhat"
    iso_nfs_dir = "/tmp/kvm_autotest_root/iso"
    nfs_server = "nfs.englab.nay.redhat.com:/pub/rhel"
    iso_nfs_server = "10.73.194.27:/vol/s2kvmauto/iso"

    # If the file path changes ,need to update  the filename
    filename = "/srv/image_iso_create/result.txt"

...
class QemuImg(storage.QemuImg):
    ...
    def create(self, params, ignore_errors=False):
        ...
        if params.get(
            ...
            external_snapshot_driver = params.get("external_snapshot_driver")
            ...
            if self.base_tag:
                if external_snapshot_driver:
                    if external_snapshot_driver == "https":
                        file_ssl_verify = params.get("file_sslverify")
                        file_readahead = params.get("file_readahead")
                        file_url = "{}://{}/{}".format(
                            external_snapshot_driver,
                        ...
                        jdict = jdict % ('\"%s\"' % external_snapshot_driver,
                                         '\"%s\"' % file_url,
                                         '\"%s\"' % file_ssl_verify,
                                         '\"%s\"' % file_readahead)
                        jdict = "\'json: {}\'".format(jdict)
                        qemu_img_cmd += " -b %s" % jdict
                    elif external_snapshot_driver == "ssh":

...
class QemuImg(storage.QemuImg):
    ...
    def create(self, params, ignore_errors=False):
        ...
        if params.get(
            ...
            external_snapshot_protocol = params.get("external_snapshot_protocol")
            ...
            if self.base_tag:
                if external_snapshot_protocol:
                    if external_snapshot_protocol == "https":
                        file_ssl_verify = params.get("file_sslverify")
                        file_readahead = params.get("file_readahead")
                        file_url = "{}://{}/{}".format(
                            external_snapshot_protocol,
                            params.get("server_host"),
                            os.path.basename(self.base_image_filename))
                        jdict = '{{"file.driver": "{}", "file.url": "{}", '
                        jdict += '"file.sslverify": "{}", "file.readahead": "{}"}}'
                        jdict = jdict.format(external_snapshot_protocol,
                                             file_url,
                                             file_ssl_verify,
                                             file_readahead)
                        jdict = "\'json: {}\'".format(jdict)
                        qemu_img_cmd += " -b %s" % jdict
                    elif external_snapshot_protocol == "ssh":
                        file_host = params.get("server_host")
                        file_path = self.base_image_filename
                        ...
                        jdict = "\'json: {}\'".format(jdict)
                        qemu_img_cmd += " -b %s" % jdict
                        qemu_img_cmd = "%s%s" % (params.get("boot_ssh_agent"),
                                                 qemu_img_cmd)
                else:

...
def run(test, params, env):
    ...
    if not re.match(r"\d+-\d+", bitmap_before_trim):
        raise exceptions.TestFail("Bitmap should be continuous before fstrim, {}".format(bitmap_before_trim))

    format_disk_cmd = params["format_disk_cmd"]
    format_disk_cmd = format_disk_cmd.replace("DISK1", guest_device_name)
    error.context("TEST STEP 4: Format disk '{:s}' in guest".format(guest_device_name), logging.info)
    session.cmd(format_disk_cmd)

    error.context("TEST STEP 5: Mount disk with discard options {:s}".format(guest_device_name),
                  logging.info)
    ...
    error.context("TEST STEP 7: Bitmap after test: {:s}".format(bitmap_after_trim), logging.info)

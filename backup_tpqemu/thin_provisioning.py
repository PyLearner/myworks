...
def run(test, params, env):
    ...
    error.context("TEST STEP 1: Boot guest with disk {:s}".format(disk_name),
                  logging.info)
    ...
    error.context(
        "TEST STEP 2: Create a file in {:s}, "
    ...
    error.context("TEST STEP 3: Bitmap before test: {:s}".format(
    ...
    error.context("TEST STEP 4: Format disk '{:s}' in guest".format(
        guest_device_name), logging.info)
    session.cmd(format_disk_cmd)

    error.context(
        "TEST STEP 5: Mount disk with discard options"
        " {:s}".format(guest_device_name), logging.info)
    ...
    if params.get(
        ...
        if not re.search(params["match_str"], o):
            raise exceptions.TestError(
                "Commond output is wrong,should be: {}.".format(
                    params["match_str"]), logging.error)

    bitmap_after_trim = get_allocation_bitmap()
    error.context("TEST STEP 7: Bitmap after test: {:s}".format(

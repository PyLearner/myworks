...
def run(test, params, env):
    ...
    def count_block_num(image_disk, gluster_mount_dir=None):
        ...
        while True:
            if params.get("enable_gluster", "no") == "yes":
                image_disk = process.system_output(
                    "basename {}".format(image_disk))
                image_disk = os.path.join(gluster_mount_dir, image_disk)
            cmd = "stat {:s}".format(image_disk)
            pattern = r"Blocks:\s(\d+)"
            try:
                block_num = re.search(
                    pattern, process.system_output(cmd)).groups()[0]
                logging.info("Block number ")
                time.sleep(wait_time)
                block_num_after_sec = re.search(
                    pattern, process.system_output(cmd)).groups()[0]
                if block_num == block_num_after_sec:
                    return block_num
                logging.info(
                    'Disk is not steady yet, block_num: {:s}, '
                    'block_num_after_{}sec:{:s}'.format(
                        block_num, wait_time, block_num_after_sec))
            except:
                error_context.context("Got block number failed,Please Check.")
    ...
    error_context.context("TEST STEP 1: Create test image disk.", logging.info)
    tempdir = ""
    ...
    if enable_gluster == "yes":
        ...
        image_file_name = "{}".format(
            gluster.get_image_filename(
                params,
                params["image_name_gluster_disk"],
                params["image_format_gluster_disk"])
        )
        params["image_name_gluster_disk"] = image_file_name
        cmd = "qemu-img create -f {} {} {}".format(
            params["image_format_gluster_disk"],
            image_file_name,
            params["image_size_gluster_disk"])
        try:
            process.system_output(cmd)
        except:
            raise exceptions.TestError(
                "Failed to create disk image in gluster server")
    else:
        ...
        object_params["image_name"] = "images/{}".format(
            params["image_name_local_disk"])
        object_params["image_format"] = params["image_format_local_disk"]
        object_params["image_size"] = params["image_size_local_disk"]
        image = qemu_storage.QemuImg(object_params,
                                     base_dir, params["image_name_local_disk"])
        image_file_name = image.create(object_params)[0]
        params["image_name_local_disk"] = os.path.splitext(image_file_name)[0]

    error_context.context("TEST STEP 2: Boot guest with image disk {:s}".format(
        image_file_name), logging.info)
    ...
    format_all_disk_cmd = format_all_disk_cmd.replace(
        "DISK1", guest_device_name)
    error_context.context("TEST STEP 3: Format disk '{:s}' in guest".format(
    ...
    error_context.context("block_num_after_format: {:s}".format(
        block_num_after_format), logging.info)

    mount_disk_cmd = params["mount_disk_cmd"]
    error_context.context("TEST STEP 4: Mount disk to {:s} in guest.".format(
        mount_disk_cmd), logging.info)
    mount_disk_cmd = mount_disk_cmd.replace("DISK1", guest_device_name)
    session.cmd(mount_disk_cmd)

    error_context.context(
        "TEST STEP 5: Create a file in the mount directory.", logging.info)
    ddfile = "{:s}/file".format(params.get("mount_point"))
    rewrite_disk_cmd = "dd if=/dev/zero of={:s} bs=1M count=2000".format(
        ddfile)
    try:
        session.cmd(rewrite_disk_cmd, timeout=timeout)
    except:
        raise exceptions.TestError("Create file in guest failed.")
    block_num_after_dd = count_block_num(image_file_name, tempdir)
    error_context.context("block_num_after_dd: {:s}".format(block_num_after_dd), logging.info)
    ...
    error_context.context("block_num_after_trim: {:s}".format(block_num_after_trim), logging.info)
    if params.get("image_format_local_disk") == "qcow2" \
            or params.get("image_format_gluster_disk") == "qcow2":
        if params.get("guest_filesystem") == "ext4":
            if block_num_after_trim != "277520":
                raise exceptions.TestFail(
                    "Expect 277520, but we got {:s}".format(
                        block_num_after_trim))
        else:
            if block_num_after_trim != "23944":
                raise exceptions.TestFail(
                    "Expect 23944, but we got {:s}".format(
                        block_num_after_trim))
    else:
        if params.get("guest_filesystem") == "ext4":
            if block_num_after_trim != "598504":
                raise exceptions.TestFail(
                    "Expect 598504, but we got {:s}".format(
                        block_num_after_trim))
        else:
            if block_num_after_trim != "20704":
                raise exceptions.TestFail(
                    "Expect 20704, but we got {:s}".format(
                        block_num_after_trim))
